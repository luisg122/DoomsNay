    ...
    $(".searchButton").click(function(){
        ...
        $.ajax({
            ...
            success:function(response){
                myArr = response
                loadCoordinates(myArr[0].coordinates.latitude, myArr[0].coordinates.longitude, myArr[0].stats.confirmed)
                buildTable(myArr)
                console.log(myArr)
            }
        })
...
function loadCoordinates(latitude, longitude, activeCases){
    ...
    container = L.map('mapid').setView(new L.LatLng(latitude, longitude), 11.5);
    ...
    var circle = L.circle([latitude, longitude], {
        ...
        fillColor: '#f03',
        fillOpacity: 0.5,
        radius: activeCases/10
    }).addTo(container);

    var str = "There are " + activeCases + " active cases";
